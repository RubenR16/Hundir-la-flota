...
public class PartidaFrame extends JFrame {
    ...
    private void procesarDisparoRecibido(String mensaje) {
        ...
        if (acierto) {
            ...
            if (barcoTocado != null && logicaJuego.verificarBarcoHundido(barcoTocado)) {
                resultadoMensaje = "RESULTADO:HUNDIDO:" + barcoTocado.filaInicial + "," + 
                           barcoTocado.columnaInicial + "," + barcoTocado.tamaño + "," + barcoTocado.horizontal;
                agregarMensaje(Messages.labels().getString("message.barcoHundidoTamanyo") + barcoTocado.tamaño + " " + 
                              Messages.labels().getString("message.fueHundido"));
        ...
        if (ultimoDisparoFila >= 0 && ultimoDisparoColumna >= 0) {
            ...
            if (resultado.equals("ACIERTO")) {
                ...
                if (partes.length > 1) {
                    ...
                    agregarMensaje(Messages.labels().getString("message.barcoHundido") + (PUNTOS_ACIERTO + PUNTOS_HUNDIDO) + " " + 
                                  Messages.labels().getString("message.puntos"));
                }
    ...
    private void procesarFinTurno() {
        ...
        actualizarEtiquetaTurno();
        actualizarEtiquetaDisparos();
        agregarMensaje(Messages.labels().getString("message.tuTurnoDisparos") + DISPAROS_POR_TURNO + " " + 
                      Messages.labels().getString("message.disparos"));
    }
    ...
    private void finalizarPartida(boolean victoria) {
        ...
        String mensaje = victoria ? 
                Messages.labels().getString("message.hasGanado") + puntuacion : 
                Messages.labels().getString("message.hasPerdido") + puntuacion;
