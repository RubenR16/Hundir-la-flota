...
public class TableroPanel extends JPanel {
    ...
    public void marcarDisparoRecibido(int fila, int columna, boolean acierto) {
        if (fila >= 0 && fila < filas && columna >= 0 && columna < columnas) {
            ...
            if (acierto) {
                // Si es acierto, marcar como tocado pero mantener visible que era un barco
                casilla.marcarTocadoConBarco(); 
            } else {
    ...
    // Método para mostrar un barco en una casilla específica (para PartidaFrame)
    public void mostrarBarcoEnCasilla(int fila, int columna, Color color) {
        if (fila >= 0 && fila < filas && columna >= 0 && columna < columnas) {
            casillas[fila][columna].colocarBarco(color);
        }
    }
