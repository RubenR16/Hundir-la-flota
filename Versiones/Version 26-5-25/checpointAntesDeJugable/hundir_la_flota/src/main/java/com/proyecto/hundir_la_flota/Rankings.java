...
public class Rankings extends JFrame {
    ...
    public List<UsuarioRanking> obtenerTop10Ranking() {
        ...
        String sql = "SELECT nombre_usuario, puntuacion FROM ranking ORDER BY puntuacion DESC LIMIT 10";

        try (PreparedStatement stmt = connection.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {

            while (rs.next()) {
                top10.add(new UsuarioRanking(rs.getString("nombre_usuario"), rs.getInt("puntuacion")));
            }
